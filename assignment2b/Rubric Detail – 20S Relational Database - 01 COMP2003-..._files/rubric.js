...
var rubricModule = {
  ...
  popupAssociationEdit : function ( url )
  {
    ...
    itemEditor = popup.launch(url+'&singlePopupPage=true&page_id='+_pageId, 'associatedItem', '1100', '500');
    if ( itemEditor )
     {
       itemEditor.focus();
     }
...
{
   var params = "toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=1,width=1100,height=675";
   var left = 0;
   var top = 0;
   ...
   var widthOffset = ( document.viewport.getWidth() - 1100 ) / 2;
...
rubricModule.refreshPanel = function( panelId, contentType, contentId, courseId, mode, associations, pointsPossibleElement, onPanelUpdatedIn, pickedRubrics )
{
  ...
  var refreshPanelUrl = "/webapps/rubric/do/course/rubricManager";
  var params =
  {
      'cmd' : 'refreshList',
      'course_id' : courseId,
      ...
      'panelId' : panelId,
      'mode' : mode,
      'pointsPossibleElement' : pointsPossibleElement
  };
  if ( associations )
  {
    ...
    params.associations = Object.toJSON( associations );
    params.pickedRubrics = pickedRubrics;
  }
...
rubricModule.removeRow = function( associatedRubricId, panelId, entityType, associatedEntityId, tableId, associationId )
{
  var row;
  var rubricRows = $$( '#rubric_' + panelId + associatedRubricId );
  if ( !rubricRows || rubricRows.size() == 0 )
  {
    return;
  }
  if ( rubricRows.size() == 1 )
  {
    row = rubricRows[0];
  }
  else if ( associationId )
  {
    // LRN-122343 handle rubric associated multiple times to the same entity
    var rowAssoc = $$( ".js-association" + associationId );
    if ( rowAssoc )
    {
      row = rowAssoc[0];
    }
       ...
       pointsPossibleElement )
  {
    var usePoints = {
       onYes: function()
       {
         ...
         $( pointsPossibleElement ).value = numberLocalizer.formatPoints( rubric.maxPoints, true /* truncate */ );
       }
    }
    /* TODO: Uncomment this code and delete the if (confirm) block
     * once we have a real title, fixed the confirm text, have button labels, and confirmation on the css styling of the modal.
     * and then refactor so rubricModule.askAndSyncRubricToPointsPossible does the same thing
     * (why do we have this same code twice?)  ugh... we've copied almost the same code
     * in both this file and ../rubric.js sigh.
    bb_dialogs.bb_confirm({
      title : 'TODO: title',
      html: page.bundle.getString( 'rubric.association.apply_to_points_possible.confirm' ),
      buttons: [
                {
                  text : 'TODO: Yes',
                  action : usePoints.onYes
              },
              {
                  text : 'TODO: No'
              } ]
    ...
    */
    if (confirm( page.bundle.getString( 'rubric.association.apply_to_points_possible.confirm' ) ) )
    {
      usePoints.onYes();
    }
  }
...
rubricModule.panel.prototype =
{
  ...
  showRubricPicker: function( )
  {
    var location = "/webapps/rubric/do/course/rubricManager?cmd=rubricPicker&context=course&course_id=" + this.courseId +
                   ...
                   "&mode=" + this.mode +
                   "&associations=" + encodeURIComponent(Object.toJSON( this.rubrics )) +
  ...
  removeRubricAssociation: function( rubricId, tableId, associationId )
  {
    ...
    if ( this.mode == "immediate" )
    {
      ...
      {
        rubricModule.removeRow( rubricId, this.panelId, this.entityType, this.entityId, tableId, associationId );
      };
      RubricDWRFacade.removeRubricAssociation( rubricId, this.entityType, this.entityId, callbackFunction.bind( this ) );
    }
    else
    {
      if ( this.removeRubric( rubricId, associationId ) )
      {
        rubricModule.removeRow( rubricId, this.panelId, this.entityType, this.entityId, tableId, associationId );
      }
    }
  ...
  {
    if ( !this.getRubric( rubricId, false, rubricAssociationId ) )
    {
  ...
  removeRubric: function( rubricId, associationId )
  {
    var rubricToRemove = this.getRubric( rubricId, false, associationId );
    if ( rubricToRemove )
    {
      if ( !rubricToRemove.withEvaluations || this.confirmRemoval( rubricToRemove ) )
      {
        this.getRubric( rubricId, true /* remove if found*/, associationId );
        this.refreshInputValue( );
        return true;
      }
    }
  ...
  getRubric: function( rubricId, remove, associationId )
  {
    ...
    for ( var i = 0, len = rubrics.length; i < len; ++i )
    {
      if ( rubrics[ i ].id == rubricId )
      {
        ...
        else if ( rubrics[ i ].rubricAssociationId == associationId )
        {
          rubric = rubrics[ i ];
        }
        if ( rubric && remove )
