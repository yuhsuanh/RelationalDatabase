...
#1. Display information of all students who have achieved the lowest grade scored in COMP2008 - display the full name (first name and  last name together) and grade. Display the records in descending order of full name. No peeking in the data. Hint: First find the lowest grade and then use it to find the students who scored the lowest grade.
...
#4 Display the course code  and the number of students enrolled in it, only for those courses that have atleast 5 students enrolled in it, in ascending order of the number of students enrolled. Give appropriate display headings. 



#5 Find the average mark for each distinct course -  display the course_code code and the average mark , rounded to an integer, with proper display headings. For example, 92.6 will round to 97 and 92.4 will round to 92 
